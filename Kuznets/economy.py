...
class Economy():
    ...
    Args:
        settings: Initial economy settings.

    Attributes:
        time: +1 every cycle.

        households: Dict of all households {hhID, household}.
        firms: Dict of all firms {firmID, firm}.
        government: Government object.

        households_data: MultiIndex dataframe for key data for each household
            (human capital, expected income, income, life_stage, spending,
            savings). Index is hhID, time, cycle (production, consumption,
            financial).
        firms_data: MultiIndex dataframe for key data for each firm (expected
            production, production, product price, revenue, inventory, labour
            cost, workers, capital investment', capital stock, marginal cost,
            debt, profit). Index is firmID, time, cycle (p, c, f).
        government_data: MultiIndex dataframe for key government data (revenue,
            expenditure, debt). Index is time, cycle (p, c, f).
        economy_data: MultiIndex dataframe for key economic data ('household
            ...
            firm profit, CPI, interest rate, unemployment rate). Concats
            household and firm data to create aggregates. Index is time, cycle
            (p, c, f).

        GDP: Calculated on production basis (total firm revenue + inventories
            valued at market price)
        interest_rate: Equivalent to monetary policy rate.
        CPI: Inflation.
        unemployment_rate:
        products: List of all products made in economy.
        ...
        for i in range(2):  # fix weird bug
            for hhID, household in self.households.items():
                self.households_data.loc[(self.time, 'p', hhID)] = {
                                    ...
                                    'Expected income':
                                    np.mean(household.expected_income),
                                    'Income': household.income,
                                    'life_stage': household.life_stage,
        ...
        for i in range(2):  # fix weird bug
            for firmID, firm in self.firms.items():
                self.firms_data.loc[(self.time, 'p', firmID)] = {
                                    'Expected production':
                                    float(firm.expected_production),
    ...
    def production(self):
        """Update production for all firms, calculate marginal cost."""
        for f in self.firms.values():
            f.update_production()
    ...
    def welfare(self):
        ...
        Welfare spending per unemployed household is 60% of average income.
        """
        # Get average incomes
        total_income = 0
        for h in self.households.values():
    ...
    def update_household_income_expectations(self, CPI):
        if CPI < 1:
            CPI = 1  # sticky wage expectations

        for h in self.households.values():
            h.expected_income.pop(0)
            if h.life_stage == 'E':
                h.expected_income.append(h.income/(1 -
                                         self.government.income_tax)
                                         * CPI)  # pretax income expectations
                h.human_capital *= 1.001
            elif h.life_stage == 'U':
                h.expected_income.append(h.income)  # pretax income
    ...
    def consumption_market(self):
        ...
        for hhID, household in self.households.items():
            ...
            for household_product in household.spending_basket:
                product_spending = household.spending * household_product
                ['Proportion']
                    ...
                    for firmID in self.products[household_product['Name']]:
                        if household_product['Price'] >= self.firms[
                            ...
                            quantity = product_spending/self.firms[
                                firmID].product_price
                            sales = self.firms[firmID].update_revenue(
                                quantity, self.firms[firmID].product_price)
                            product_spending -= sales

                            # Firms adjust pricing based on sales.
                            # Firm increases price linearly until max 3% gain
                            # if everything sold.
                            max_price_gain = self.firms[
                                firmID].max_price_gain  # 3%
                            self.firms[firmID].product_price = self.firms[
                                firmID].product_price * (1 + max_price_gain
                                                         * sales/self.firms[
                                                            firmID].expected_production)
    ...
    def update_households_data(self, households_data, households, time, cycle):
        ...
        new_data = [pd.DataFrame({
            ...
            'life_stage': str(household.life_stage),
    ...
    def update_economy_data(self, cycle):
        ...
        self.government_data = pd.concat([self.government_data,
                                          pd.DataFrame({
                                            'Revenue':
                                            float(self.government.revenue),
                                            'Expenditure':
                                            float(self.government.expenditure),
                                            'Debt':
                                            float(self.government.debt)},
    ...
    def cycle(self, number=1):
        ...
        for i in range(number):
            ...
            self.income_tax()
            self.welfare()
            self.update_household_income_expectations(
                self.growth_rate(self.economy_data['CPI']))
